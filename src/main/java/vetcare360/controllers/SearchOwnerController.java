...
public class SearchOwnerController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        filteredOwners = new FilteredList<>(ownersList, p -> true);
    ...
    private void addNewOwner() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/views/AddOwner.fxml"));
            Parent view = loader.load();

            AddOwnerController controller = loader.getController();
            controller.setMainContainer((BorderPane) ownersTable.getScene().getRoot());
            ...
            alert.setHeaderText("Failed to load Add Owner form.");
    ...
    private void filterOwners() {
        String filterText = lastNameField.getText().toLowerCase().trim();
        ...
        filteredOwners.setPredicate(owner -> {
            ...
            return owner.getLastName().toLowerCase().contains(filterText);
        });
    }
    ...
    private void populateSampleData() {
        ...
        ownersList.get(6).addPet(new Pet("9", "Lucky", LocalDate.of(2015, 1, 30), "Bird"));
